...
def train_classreg(model, loss_func, opt, scheduler, train_loader, dev, epoch, steps_per_epoch=None, grad_scaler=None, tb_helper=None):
    ...
    with tqdm.tqdm(train_loader) as tq:
        for X, y_cat, y_reg, y_domain, _, y_cat_check, y_domain_check in tq:
            ...
            if torch.is_tensor(label_cat) and torch.is_tensor(model_output_cat) and np.iterable(label_cat) and np.iterable(model_output_cat) and model_output_cat.nelement():
            ...
            if num_domains == 1:
                if torch.is_tensor(label_domain) and torch.is_tensor(model_output_domain) and np.iterable(label_domain) and np.iterable(model_output_domain) and model_output_domain.nelement():
            ...
            ## multiple domain regions
                ...
                for idx, (k,v) in enumerate(y_domain_check.items()):                    
                    ...
                    if not torch.is_tensor(pred_domain) or not np.iterable(pred_domain) or not model_output_domain.nelement(): continue;
...
def evaluate_classreg(model, test_loader, dev, epoch, for_training=True, loss_func=None, steps_per_epoch=None, tb_helper=None, eval_attack=None, eps_attack=None,
    ...
    scores_cat, scores_reg, indexes_cat, scores_attack = [], [], [], [];
    ...
    with torch.no_grad():
        with tqdm.tqdm(test_loader) as tq:
			...
			inputs_attack = [fngm_attack(element,eps_attack,input_eps_min[idx].to(dev,non_blocking=True),input_eps_max[idx].to(dev,non_blocking=True)).detach().to(dev,non_blocking=True) for idx,element in enumerate(inputs)]
                    else:
                        inputs_attack = [fgsm_attack(element,eps_attack,input_eps_min[idx].to(dev,non_blocking=True),input_eps_max[idx].to(dev,non_blocking=True)).detach().to(dev,non_blocking=True) for idx,element in enumerate(inputs)]
