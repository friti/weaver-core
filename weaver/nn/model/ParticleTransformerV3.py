...
class ParticleTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if fc_params is not None:
            ...
            for out_dim, drop_rate in fc_params:
                fcs_class.append(nn.Sequential(nn.Linear(in_dim, out_dim),
                                               nn.BatchNorm1d(out_dim),
                                               nn.GELU() if activation == 'gelu' else nn.ReLU(),
                                               nn.Dropout(drop_rate))
                )
                fcs_regre.append(nn.Sequential(nn.Linear(in_dim, out_dim),
                                               nn.BatchNorm1d(out_dim),
                                               nn.GELU() if activation == 'gelu' else nn.ReLU(),
                                               nn.Dropout(drop_rate))
                )
                ...
                in_dim = out_dim
            fcs_class.append(nn.Linear(in_dim, num_classes))
            fcs_regre.append(nn.Linear(in_dim, num_targets))
            self.fc_class = nn.Sequential(*fcs_class)
            self.fc_regre = nn.Sequential(*fcs_regre)
        else:
            self.fc_class = None
            self.fc_regre = None
    ...
    def forward(self, x, v=None, mask=None, uu=None, uu_idx=None):
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            output_regre = self.fc_regre(x_cls)
            
            if self.for_inference:
                if self.num_classes and not self.num_targets:
                    output = torch.softmax(output_class, dim=1)                    
                elif self.num_classes and self.num_targets:
                    output_class = torch.softmax(output_class,dim=1);
                    output_regere = torch.softmax(output_regre,dim=1);
                    output = torch.cat((output_class,output_regre),dim=1);
            elif self.num_domains and self.fc_domain:                
                if not self.split_domain_outputs:
                    output_domain = self.fc_domain(x_cls)
                    output = torch.cat((output_class,output_regre,output_domain),dim=1);
                else:
                    output = torch.cat((output_class,output_regre),dim=1);
